...
type Article struct {
	...
	Title       string `gorm:"size:20;not null;unique;unique_index;"` // 标题
	Author      string `gorm:"size:15;not null;index:author;"`        // 作者
	IsShow      int    `gorm:"type:enum(1,0);default:0"`              // 1是显示，默认显示
